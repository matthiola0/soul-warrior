...
export default class Boss extends cc.Component {
    ...
    private dashCooldown: number = 5.0;
    private enemyhealth: number = 100; // Boss血量
    ...
    bossAttack() {
        // 在这个函数中，Boss会向玩家的方向发起冲锋
        let direction = this.player.node.position.sub(this.node.position).normalize();
        let distance = 300;  // 调整这个值以改变Boss冲锋的距离
        let speed = 0.2;     // 调整这个值以改变Boss冲锋的速度
        let targetPosition = this.node.position.add(direction.mul(distance));

        this.isDashing = true; //设置为冲锋状态
        this.anim.play('boss1_hit');

        cc.tween(this.node)
            .to(speed, { position: targetPosition })
            .call(() => {
                // Boss冲锋结束后，可以执行一些代码，例如启动一次攻击
                // this.createAttackEffect();
                //开始计时，冷却时间结束后再将isDashing设为false
                this.scheduleOnce(() => { this.isDashing = false; }, this.dashCooldown);
    ...
    // createAttackEffect() {
    //     let effect = cc.instantiate(this.attackEffectPrefab);
    //     effect.position = this.player.node.position;
